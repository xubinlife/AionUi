...
describe('fontSizes', () => {
  it('exposes all region keys with sane defaults', () => {
    expect(FONT_SIZE_KEYS).toEqual(['app', 'chat', 'markdown', 'code']);
    expect(defaultFontSizes()).toEqual({ app: 14, chat: 14, markdown: 13, code: 12 });
  });
